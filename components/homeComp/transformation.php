<!-- Beyond Development / Transformation Services Section with Ultra-Smooth Physics LERP Scroll Animations -->
<section id="transformation-scroll-container" class="relative w-full lg:h-[200vh] bg-[#FBFBFC]">

  <!-- Sticky Viewport Area (Active on lg+ screens) -->
  <div class="lg:sticky lg:top-[75px] lg:min-h-[calc(100vh-75px)] w-full flex flex-col justify-center py-4 sm:py-6 lg:py-8 overflow-hidden">
    
    <div class="contain h-full flex flex-col justify-center gap-5 lg:gap-6 my-auto">

      <!-- Top Header -->
      <div class="text-center max-w-4xl mx-auto">
        <h2 class="Abhaya text-2xl sm:text-3xl md:text-4xl lg:text-[40px] xl:text-[44px] font-extrabold text-black leading-tight tracking-tight">
          Beyond Development.<br />
          <span class="font-extrabold text-black">We Deliver <span class="text-[#ffc835]">Transformation.</span></span>
        </h2>
      </div>

      <!-- 4-Column Cards Wrapper -->
      <div class="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3.5 lg:gap-4 xl:gap-5 items-stretch">

        <!-- Column 1 (Base - Anchored) -->
        <div id="scroll-card-1" class="w-full will-change-transform">
          <div class="group h-full min-h-[330px] sm:min-h-[350px] flex flex-col justify-between bg-white border border-[#E5E7EB] hover:border-[#ffc835] rounded-[22px] p-5 sm:p-6 transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
            
            <!-- Top: Icon & Content -->
            <div>
              <div class="w-11 h-11 rounded-xl bg-[#F3F4F6] group-hover:bg-[#ffc835] flex items-center justify-center mb-4 transition-all duration-300 shadow-sm">
                <svg class="w-5 h-5 text-[#111827]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M9.75 3.104v5.714a2.25 2.25 0 01-.659 1.591L5 14.322V19a2.25 2.25 0 002.25 2.25h9.5A2.25 2.25 0 0019 19v-4.678l-4.091-3.913a2.25 2.25 0 01-.659-1.591V3.104" />
                  <path stroke-linecap="round" stroke-linejoin="round" d="M9 3h6M10 9h4" />
                </svg>
              </div>

              <h3 class="text-[17.5px] lg:text-[18.5px] font-bold text-[#111827] leading-snug mb-2.5">
                Strategic Technology<br class="hidden xl:inline" /> Consulting
              </h3>

              <p class="text-[14px] text-[#4B5563] leading-relaxed font-normal line-clamp-4">
                From architecture audits to digital transformation strategy, we help leaders align technology with business goals for measurable ROI.
              </p>
            </div>

            <!-- Bottom: Button -->
            <div class="pt-4 mt-4 border-t border-gray-100">
              <a href="custom-software-solution.html" class="inline-flex items-center justify-center w-full px-4 py-2 rounded-full border border-[#111827] text-[#111827] text-[12px] font-semibold transition-all duration-300 hover:bg-[#ffc835] hover:border-[#ffc835] hover:text-black group/btn">
                <span>View Consulting Services</span>
                <svg class="w-3.5 h-3.5 ml-1.5 transition-transform duration-300 group-hover/btn:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3" />
                </svg>
              </a>
            </div>

          </div>
        </div>


        <!-- Column 2 (Smooth Fluid Glide Up) -->
        <div id="scroll-card-2" class="w-full lg:translate-y-[220px] lg:opacity-20 will-change-transform">
          <div class="group h-full min-h-[330px] sm:min-h-[350px] flex flex-col justify-between bg-white border border-[#E5E7EB] hover:border-[#ffc835] rounded-[22px] p-5 sm:p-6 transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
            
            <!-- Top: Icon & Content -->
            <div>
              <div class="w-11 h-11 rounded-xl bg-[#F3F4F6] group-hover:bg-[#ffc835] flex items-center justify-center mb-4 transition-all duration-300 shadow-sm">
                <svg class="w-5 h-5 text-[#111827]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M17.25 6.75L22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3l-4.5 16.5" />
                </svg>
              </div>

              <h3 class="text-[17.5px] lg:text-[18.5px] font-bold text-[#111827] leading-snug mb-2.5">
                Digital Product<br class="hidden xl:inline" /> Development & Engineering
              </h3>

              <p class="text-[14px] text-[#4B5563] leading-relaxed font-normal line-clamp-4">
                As your digital product engineering company, we handle the full lifecycle of software development—from cloud-native applications to complex ERP systems.
              </p>
            </div>

            <!-- Bottom: Button -->
            <div class="pt-4 mt-4 border-t border-gray-100">
              <a href="web-design-development.html" class="inline-flex items-center justify-center w-full px-4 py-2 rounded-full border border-[#111827] text-[#111827] text-[12px] font-semibold transition-all duration-300 hover:bg-[#ffc835] hover:border-[#ffc835] hover:text-black group/btn">
                <span>View Product Engineering</span>
                <svg class="w-3.5 h-3.5 ml-1.5 transition-transform duration-300 group-hover/btn:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3" />
                </svg>
              </a>
            </div>

          </div>
        </div>


        <!-- Column 3 (Smooth Fluid Glide Up - Wave 2) -->
        <div id="scroll-card-3" class="w-full lg:translate-y-[360px] lg:opacity-10 will-change-transform">
          <div class="group h-full min-h-[330px] sm:min-h-[350px] flex flex-col justify-between bg-white border border-[#E5E7EB] hover:border-[#ffc835] rounded-[22px] p-5 sm:p-6 transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
            
            <!-- Top: Icon & Content -->
            <div>
              <div class="w-11 h-11 rounded-xl bg-[#F3F4F6] group-hover:bg-[#ffc835] flex items-center justify-center mb-4 transition-all duration-300 shadow-sm">
                <svg class="w-5 h-5 text-[#111827]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 00-2.456 2.456zM16.894 20.567L16.5 21.75l-.394-1.183a2.25 2.25 0 00-1.423-1.423L13.5 18.75l1.183-.394a2.25 2.25 0 001.423-1.423l.394-1.183.394 1.183a2.25 2.25 0 001.423 1.423l1.183.394-1.183.394a2.25 2.25 0 00-1.423 1.423z" />
                </svg>
              </div>

              <h3 class="text-[17.5px] lg:text-[18.5px] font-bold text-[#111827] leading-snug mb-2.5">
                AI, Data and<br class="hidden xl:inline" /> Analytics Solutions
              </h3>

              <p class="text-[14px] text-[#4B5563] leading-relaxed font-normal line-clamp-4">
                We integrate advanced AI, generative AI, and machine learning models to automate operations, predict trends, and personalize customer experiences at scale.
              </p>
            </div>

            <!-- Bottom: Button -->
            <div class="pt-4 mt-4 border-t border-gray-100">
              <a href="mobile-application.html" class="inline-flex items-center justify-center w-full px-4 py-2 rounded-full border border-[#111827] text-[#111827] text-[12px] font-semibold transition-all duration-300 hover:bg-[#ffc835] hover:border-[#ffc835] hover:text-black group/btn">
                <span>View AI Solutions</span>
                <svg class="w-3.5 h-3.5 ml-1.5 transition-transform duration-300 group-hover/btn:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3" />
                </svg>
              </a>
            </div>

          </div>
        </div>


        <!-- Column 4 (Smooth Fluid Glide Up - Wave 3) -->
        <div id="scroll-card-4" class="w-full lg:translate-y-[480px] lg:opacity-0 will-change-transform">
          <div class="group h-full min-h-[330px] sm:min-h-[350px] flex flex-col justify-between bg-white border border-[#E5E7EB] hover:border-[#ffc835] rounded-[22px] p-5 sm:p-6 transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
            
            <!-- Top: Icon & Content -->
            <div>
              <div class="w-11 h-11 rounded-xl bg-[#F3F4F6] group-hover:bg-[#ffc835] flex items-center justify-center mb-4 transition-all duration-300 shadow-sm">
                <svg class="w-5 h-5 text-[#111827]" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m0-10.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.75c0 5.592 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.57-.598-3.75h-.152c-3.196 0-6.1-1.25-8.25-3.286zm0 13.036h.008v.008H12v-.008z" />
                </svg>
              </div>

              <h3 class="text-[17.5px] lg:text-[18.5px] font-bold text-[#111827] leading-snug mb-2.5">
                Cloud Operations<br class="hidden xl:inline" /> and Cybersecurity
              </h3>

              <p class="text-[14px] text-[#4B5563] leading-relaxed font-normal line-clamp-4">
                As your IT service company, we engineer cloud-native environments rooted in Zero Trust principles, ensuring your infrastructure is as scalable as it is impenetrable.
              </p>
            </div>

            <!-- Bottom: Button -->
            <div class="pt-4 mt-4 border-t border-gray-100">
              <a href="web-hosting.html" class="inline-flex items-center justify-center w-full px-4 py-2 rounded-full border border-[#111827] text-[#111827] text-[12px] font-semibold transition-all duration-300 hover:bg-[#ffc835] hover:border-[#ffc835] hover:text-black group/btn">
                <span>View Cybersecurity</span>
                <svg class="w-3.5 h-3.5 ml-1.5 transition-transform duration-300 group-hover/btn:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3" />
                </svg>
              </a>
            </div>

          </div>
        </div>

      </div>

      <!-- Bottom Center "View All Services" Button -->
      <div class="text-center pt-2 flex justify-center relative z-20">
        <a href="start-project.php">
          <button
            class="bg-[#0f172a] text-white text-[13px] xl:text-[13.5px] px-6 py-2.5 rounded-full inline-flex items-center justify-center transition-all duration-300 ease-in-out hover:bg-[#ffc835] hover:text-black hover:shadow-lg font-medium shadow-md">
            View All Services
            <span class="pl-2">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                stroke-width="1.8" stroke="currentColor" class="w-3.5 h-3.5">
                <path stroke-linecap="round" stroke-linejoin="round"
                  d="m4.5 19.5 15-15m0 0H8.25m11.25 0v11.25" />
              </svg>
            </span>
          </button>
        </a>
      </div>

    </div>

  </div>

</section>

<!-- Physics-based LERP Smooth Scroll Engine -->
<script>
  (function () {
    const container = document.getElementById("transformation-scroll-container");
    const card2 = document.getElementById("scroll-card-2");
    const card3 = document.getElementById("scroll-card-3");
    const card4 = document.getElementById("scroll-card-4");

    if (!container) return;

    let targetProgress = 0;
    let currentProgress = 0;
    let isAnimating = false;

    function calcTargetProgress() {
      if (window.innerWidth < 1024) {
        return 0;
      }
      const rect = container.getBoundingClientRect();
      const totalScrollable = container.offsetHeight - window.innerHeight;
      if (totalScrollable <= 0) return 0;

      const scrolled = -rect.top;
      return Math.min(Math.max(scrolled / totalScrollable, 0), 1);
    }

    // Smoothstep cubic easing: 3x^2 - 2x^3
    function smoothstep(t) {
      return t * t * (3 - 2 * t);
    }

    function renderLoop() {
      if (window.innerWidth < 1024) {
        if (card2) { card2.style.transform = "none"; card2.style.opacity = "1"; }
        if (card3) { card3.style.transform = "none"; card3.style.opacity = "1"; }
        if (card4) { card4.style.transform = "none"; card4.style.opacity = "1"; }
        isAnimating = false;
        return;
      }

      // Physics damping interpolation (Faster & responsive LERP factor)
      const diff = targetProgress - currentProgress;
      currentProgress += diff * 0.14;

      if (Math.abs(diff) < 0.0003) {
        currentProgress = targetProgress;
      }

      // Card 2 Glide (starts at 0.03, reaches top at 0.38)
      const p2 = Math.min(Math.max((currentProgress - 0.03) / 0.35, 0), 1);
      const e2 = smoothstep(p2);
      const y2 = (1 - e2) * 260;
      const op2 = 0.25 + (e2 * 0.75);
      if (card2) {
        card2.style.transform = `translate3d(0, ${y2.toFixed(2)}px, 0)`;
        card2.style.opacity = op2.toFixed(3);
      }

      // Card 3 Glide (starts at 0.20, reaches top at 0.65)
      const p3 = Math.min(Math.max((currentProgress - 0.20) / 0.45, 0), 1);
      const e3 = smoothstep(p3);
      const y3 = (1 - e3) * 420;
      const op3 = 0.15 + (e3 * 0.85);
      if (card3) {
        card3.style.transform = `translate3d(0, ${y3.toFixed(2)}px, 0)`;
        card3.style.opacity = op3.toFixed(3);
      }

      // Card 4 Glide (starts at 0.42, reaches top at 0.88)
      const p4 = Math.min(Math.max((currentProgress - 0.42) / 0.46, 0), 1);
      const e4 = smoothstep(p4);
      const y4 = (1 - e4) * 580;
      const op4 = 0.05 + (e4 * 0.95);
      if (card4) {
        card4.style.transform = `translate3d(0, ${y4.toFixed(2)}px, 0)`;
        card4.style.opacity = op4.toFixed(3);
      }

      if (Math.abs(targetProgress - currentProgress) > 0.0003) {
        requestAnimationFrame(renderLoop);
      } else {
        isAnimating = false;
      }
    }

    function onScrollOrResize() {
      targetProgress = calcTargetProgress();
      if (!isAnimating) {
        isAnimating = true;
        requestAnimationFrame(renderLoop);
      }
    }

    window.addEventListener("scroll", onScrollOrResize, { passive: true });
    window.addEventListener("resize", onScrollOrResize, { passive: true });

    if (window.lenis) {
      window.lenis.on('scroll', onScrollOrResize);
    }

    document.addEventListener("DOMContentLoaded", () => {
      targetProgress = calcTargetProgress();
      currentProgress = targetProgress;
      if (window.lenis) {
        window.lenis.on('scroll', onScrollOrResize);
      }
      onScrollOrResize();
    });

    setTimeout(() => {
      targetProgress = calcTargetProgress();
      currentProgress = targetProgress;
      if (window.lenis) {
        window.lenis.on('scroll', onScrollOrResize);
      }
      onScrollOrResize();
    }, 200);
  })();
</script>
